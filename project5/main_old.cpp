...
int main( int argc, char ** argv ) {
    ...
    while (scanf("%c", &instruction) != EOF) {
        ...
        case 'b':
            result = begin_trx();
            printf("begin_trx result : %d\n", result);
            break;
        case 'e':
            scanf("%d", &num);
            result = end_trx(num);
            printf("end_trx result : %d\n", result);
            break;
        case 'i':
            key = 0;
            memset(value, 0, 120);
        ...
        case 'f':
            key = 0;
            memset(value, 0, 120);
            scanf("%d %ld %d", &num, &key, &trx_id);
            result = db_find(num, key, value, trx_id); // trx_id!!
        ...
        case 'u':
            key = 0;
            memset(value, 0, 120);
            scanf("%d %ld %s %d", &num, &key, value, &trx_id);
            result = db_update(num, key, value, trx_id); // trx_id!!
