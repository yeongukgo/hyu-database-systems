...
// Wrapper function of buffer_request_page().
int index_request_page(int table_id, pagenum_t pagenum, buffer_block_t** dest) {
    return buffer_request_page(table_id, pagenum, dest);
}

// Wrapper function of buffer_release_page().
int index_release_page(buffer_block_t* buf_block, bool is_writed) {
    return buffer_release_page(buf_block, is_writed);
}
...
int index_get_index(buffer_block_t *parent_buffer, pagenum_t current_pagenum) {
    page_t *parent_page = &(parent_buffer->frame);
    int i;

    if(parent_page->right_sibling_or_one_more_page_num == current_pagenum) {
        return GET_INDEX_LEFTMOST; // Given page is the leftmost child.
    }
    for(i = 0; i < parent_page->num_of_keys; i++) {
        if(parent_page->childs.entries[i].pagenum == current_pagenum) {
            return i;
        }
    }
...
buffer_block_t* find_leftmost_leaf(buffer_block_t *header_buffer) {
    ...
    page_t *current_page;
    int table_id = header_buffer->table_id;

    header_page = (header_page_t*)&(header_buffer->frame);

    current_pagenum = header_page->root_page_num;
    
    if(current_pagenum == 0) {
        return NULL; // Empty table.
    }

    buffer_request_page(table_id, current_pagenum, &current_buffer);
    current_page = &(current_buffer->frame);

    // Find leftmost leaf page in table.
    while(!current_page->is_leaf) {
        current_pagenum = current_page->right_sibling_or_one_more_page_num;
        buffer_release_page(current_buffer, false);
        buffer_request_page(table_id, current_pagenum, &current_buffer);
        current_page = &(current_buffer->frame);
    }
...
pagenum_t index_find_leaf(int table_id, int64_t key) {
    ...
    while(!current_page->is_leaf) {
        ...
        /*
        left = 0;
        right = current_page->num_of_keys - 1;
        while(left <= right) {
            mid = (left + right) / 2;
            if(key >= current_page->childs.entries[mid].key) {
                left = mid + 1;
            }
            else {
                right = mid - 1;
...
int index_insert_into_parent(buffer_block_t* left_buffer, int64_t key, buffer_block_t* right_buffer) {
    ...
    left_index = index_get_index(parent_buffer, left_buffer->page_num);
...
int index_remove_entry_from_internal(buffer_block_t *current_buffer) {
    ...
    parent_page = &(parent_buffer->frame);
    current_index = index_get_index(parent_buffer, current_pagenum);
...
int index_merge_pages(buffer_block_t *current_buffer, buffer_block_t *neighbor_buffer, buffer_block_t *parent_buffer,
    ...
    // If current page has a neighbor to the left,
        ...
        if(!current_page->is_leaf) {
            ...
            neighbor_page->childs.entries[neighbor_page->num_of_keys].pagenum = current_page->right_sibling_or_one_more_page_num;
            buffer_request_page(table_id, neighbor_page->childs.entries[neighbor_page->num_of_keys].pagenum, &temp_buffer);
            temp_page = &(temp_buffer->frame);
            temp_page->parent_or_next_free_page_num = neighbor_pagenum;
            buffer_release_page(temp_buffer, true);
            neighbor_page->num_of_keys++;
...
int index_delete_entry(buffer_block_t *current_buffer) {
    ...
    // Find neighbor page.
    neighbor_index = index_get_index(parent_buffer, current_pagenum) - 1;
    if(neighbor_index < -2) return -2; // error.
