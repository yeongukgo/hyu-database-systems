...
int evict_buffer_block(buffer_block_t* buf_block) {
    ...
    delete_from_hash_table(buf_block->table_id, buf_block->page_num);
...
int create_buffer_block(int table_id, pagenum_t pagenum, page_t buf_page, buffer_block_t **dest) {
    ...
    insert_to_hash_table(table_id, pagenum, buf_block);
...
int insert_to_hash_table(int table_id, pagenum_t pagenum, buffer_block_t *buf_block) {
    ...
    pos = key % hash_size;
    hash_buc = &(hash_table[pos]);

    if(hash_buc->table_id != -1) {
        while(hash_buc->next != NULL) {
            hash_buc = hash_buc->next;
        }
        hash_buc->next = (hash_bucket_t*)malloc(sizeof(hash_bucket_t));
        hash_buc = hash_buc->next;
    }
    hash_buc->table_id = table_id;
    hash_buc->pagenum = pagenum;
    hash_buc->buf_block = buf_block;
    hash_buc->next = NULL;
...
buffer_block_t* find_from_hash_table(int table_id, pagenum_t pagenum) {
    ...
    while(hash_buc->table_id != table_id || hash_buc->pagenum != pagenum) {
        hash_buc = hash_buc->next;
        if(hash_buc == NULL) {
            return NULL; // not exist.
        }
    ...
    // Add pin.
    hash_buc->buf_block->is_pinned++;

    return hash_buc->buf_block;
}

int delete_from_hash_table(int table_id, pagenum_t pagenum) {
    hash_bucket_t* hash_buc, *hash_buc_temp;
    uint64_t key;
    int pos;
    ...
    if(hash_buc->table_id == table_id && hash_buc->pagenum == pagenum) {
        if(hash_buc->next != NULL) {
            hash_buc_temp = hash_buc->next;
            hash_table[pos] = *hash_buc_temp;
            free(hash_buc_temp);
        }
        else {
            hash_buc->table_id = -1;
            hash_buc->buf_block = NULL;
        }
    }
    else {
        if(hash_buc->next == NULL) {
        ...
        while(hash_buc->next->table_id != table_id || hash_buc->next->pagenum != pagenum) {
            hash_buc = hash_buc->next;
            if(hash_buc->next == NULL) {
                return -1; // not exist.
            }
        ...
        hash_buc_temp = hash_buc->next;
        hash_buc->next = hash_buc_temp->next;
        free(hash_buc_temp);
    }
...
int buffer_init_db(int buf_num) {
    ...
    hash_size = buf_num;
    hash_table = (hash_bucket_t*)malloc(sizeof(hash_bucket_t) * hash_size);
...
int buffer_request_page(int table_id, pagenum_t pagenum, buffer_block_t** dest) {
    ...
    //buf_block->is_pinned++;
    // Pin was added at find_from_hash_table or create_buffer_block.
    *dest = buf_block;
    return 0;
}
